...
def block_until_stopped(
    ...
    *,
    ...
    """Block the foreground run until Ctrl-C, a graceful `stop` marker, or timeout.
    ...
    / fall-through emits `orchestrator_stopped{reason:"user"}`; a `timeout` deadline
    emits `{reason:"timeout"}` — the primary liveness backstop for autonomous runs.

    On a terminal stop, when `kill_panes` (autonomous) or the reason is `timeout`,
    every pane is killed so a stuck teammate stops draining the shared quota.
    `timeout=None` preserves the infinite foreground block.
    """
    ...
    try:
        ...
        if not no_block and (kill_panes or reason == "timeout"):
