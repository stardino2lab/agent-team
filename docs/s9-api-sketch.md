...
| Multi-CLI extension (D11) | Light isolation via `agent_team.cli_registry` (leaf module): `CliSpec` entries + `is_lead_supported` / `is_teammate_supported` lookup. `personas.py`, `spawn_approval.py`, and `orchestrator.py` all derive cli validation from the registry. Today only `claude` has `supports_lead=True`; `codex` is teammate-only; `antigravity` is unregistered (S11). Configuring `lead_cli: codex|antigravity|anything-else` raises `LeadCliNotSupportedError` from `Orchestrator.start` before any disk state is created. S11+ adds the codex/antigravity entries + their launch builders. No Protocol / ABC; the inner `_build_lead_launch_command` stays an elif. |
...
| 7 | `tests/unit/test_orchestrator.py::test_unsupported_lead_cli_raises_registry_error` | passing `cli="codex"` / `"antigravity"` / `"xyz"` to `_build_lead_launch_command` raises `LeadCliNotSupportedError` mentioning S11 |
